console.info("Price King Stock Tracker Build: 4.0");
...
const ROLES = {
  admin: {
    label: "Administrator",
    sections: ["overview", "alcohol", "stock", "history", "audit"],
    writeInventory: true
  },
  alcohol_manager: {
    label: "Alcohol Manager",
    sections: ["overview", "alcohol", "stock", "history"],
    writeInventory: true
  },
  alcohol_viewer: {
...
function canUseSection(section) {
  ...
  if (entryMode === "alcohol") return ["overview", "alcohol", "stock", "history"].includes(section);
  return true;
}
...
async function loadBundledCatalogue() {
  if (state.catalogReady || state.catalogFallbackAttempted) return;
  state.catalogFallbackAttempted = true;

  try {
    const response = await fetch(BUNDLED_CATALOG_URL, { cache: "no-store" });
    if (!response.ok) throw new Error(`Master list could not be loaded (${response.status}).`);
    ...
    state.catalog = catalogueFromWorkbook(workbook);
    state.catalogSource = "Included Master Alcohol List";
...
function catalogueSearch(query, limit = 18) {
  ...
  for (const item of state.catalog) {
    const haystack = `${item.name} ${item.brand} ${item.size} ${item.category} ${item.style}`.toLowerCase();
    if (!words.every((word) => haystack.includes(word))) continue;

    let score = 5;
    if (item.name.toLowerCase().startsWith(clean)) score = 0;
    else if (item.brand.toLowerCase().startsWith(clean)) score = 1;
    else if (item.name.toLowerCase().includes(clean)) score = 2;
    else if (item.brand.toLowerCase().includes(clean)) score = 3;
      ...
      <div class="inventory-header">
        <div class="section-heading">
          <div>
            ...
            <p>
              Count date: <strong>${escapeHtml(formatDate(state.settings.countDate))}</strong>
              · Master list:
...
function openItemModal(item = null) {
  ...
  const current = sanitizeItem(item || {
    ...
    looseUnits: 0,
    reorderLevel: 0,
    caseMarkup: state.settings.defaultMarkup,
    unitMarkup: state.settings.defaultMarkup
  });
      ...
      <div class="modal-body">
        <div class="form-grid">
          <div class="field full-span product-combobox">
            <label for="itemName">Select or type product <em>*</em></label>
            <input
              id="itemName"
              type="text"
              value="${escapeHtml(current.name)}"
              placeholder="Start typing a product or brand, then press Enter"
              autocomplete="off"
              role="combobox"
          ...
          <section class="quantity-box full-span">
            ...
            <div class="form-grid three">
              <div class="field">
                ...
                <input id="itemCases" type="number" min="0" step="1" value="${current.cases}">
              </div>
              <div class="field">
                <label for="itemUnitsPerCase">Units in each case</label>
                <input id="itemUnitsPerCase" type="number" min="0" step="1" value="${current.unitsPerCase}">
              </div>
              <div class="field">
                <label for="itemLooseUnits">Loose units</label>
                <input id="itemLooseUnits" type="number" min="0" step="1" value="${current.looseUnits}">
              </div>
            </div>
          ...
          <section class="price-box full-span">
            ...
            <div class="pricing-grid">
              <div class="field">
                ...
                <input id="itemCaseCost" type="number" min="0" step="0.01" value="${current.caseCost || ""}" placeholder="0.00">
              </div>

              <div class="field">
                <label for="itemCaseMarkup">Case markup %</label>
                <input id="itemCaseMarkup" type="number" min="0" step="0.01" value="${currentCalc.caseMarkup || current.caseMarkup || state.settings.defaultMarkup}">
              </div>

              <div class="field">
                <label for="itemCaseSelling">Selling price per case</label>
                <input id="itemCaseSelling" type="number" min="0" step="0.01" value="${current.caseSellingPrice || ""}" placeholder="0.00">
              </div>
            ...
            <div class="calc-strip">
              <div class="calc-item"><span>Case Cost</span><strong id="formCaseCost">${formatMoney(0)}</strong></div>
              <div class="calc-item"><span>Case Selling</span><strong id="formCaseSelling">${formatMoney(0)}</strong></div>
              <div class="calc-item"><span>Profit Per Case</span><strong id="formCaseProfit">${formatMoney(0)}</strong></div>
            </div>
          </section>
          ...
          <section class="price-box full-span">
            ...
            <div class="pricing-grid">
              <div class="field">
                ...
                <input id="itemUnitCost" type="number" min="0" step="0.01" value="${current.unitCost || ""}" placeholder="Auto from case cost">
              </div>

              <div class="field">
                <label for="itemUnitMarkup">Unit markup %</label>
                <input id="itemUnitMarkup" type="number" min="0" step="0.01" value="${currentCalc.unitMarkup || current.unitMarkup || state.settings.defaultMarkup}">
              </div>

              <div class="field">
                <label for="itemSellingPrice">Selling price for one</label>
                <input id="itemSellingPrice" type="number" min="0" step="0.01" value="${current.sellingPrice || ""}" placeholder="0.00">
              </div>
            ...
            <div class="calc-strip">
              ...
              <div class="calc-item"><span>Unit Selling</span><strong id="formUnitSelling">${formatMoney(0)}</strong></div>
          ...
          <section class="quantity-box full-span">
            <div class="calc-strip total-preview-strip">
              <div class="calc-item"><span>Total Stock Cost</span><strong id="formStockCost">${formatMoney(0)}</strong></div>
              <div class="calc-item"><span>Total Sales Value</span><strong id="formSalesValue">${formatMoney(0)}</strong></div>
          ...
          <div class="field">
            ...
            <input id="itemReorder" type="number" min="0" step="1" value="${current.reorderLevel}">
          </div>
  ...
  productInput.addEventListener("focus", renderSuggestions);
  ...
  const preview = () => {
    ...
    $("formCaseCost").textContent = formatMoney(calc.caseCostValue);
    $("formCaseSelling").textContent = formatMoney(calc.caseSellingValue);
    $("formCaseProfit").textContent = formatMoney(calc.caseProfit);

    $("formUnitCost").textContent = formatMoney(calc.costEach);
    $("formUnitSelling").textContent = formatMoney(calc.sellingEach);
    $("formUnitProfit").textContent = formatMoney(calc.unitProfit);

    $("formStockCost").textContent = formatMoney(calc.stockCost);
    $("formSalesValue").textContent = formatMoney(calc.salesValue);
  ...
  $("applyCaseMarkup").addEventListener("click", () => {
    ...
    const markup = nonNegative($("itemCaseMarkup").value || state.settings.defaultMarkup);
  ...
  $("applyUnitMarkup").addEventListener("click", () => {
    ...
    const markup = nonNegative($("itemUnitMarkup").value || state.settings.defaultMarkup);
  ...
  $("itemForm").addEventListener("submit", async (event) => {
    ...
    try {
      ...
      await addAudit(
        editing ? "item_updated" : "item_added",
        `${editing ? "Updated" : "Added"} ${name}`,
...
function renderStockTracker() {
  ...
  const filteredItems = state.items.filter((item) => {
    ...
    const haystack = `${item.name} ${item.size} ${item.category} ${item.supplier}`.toLowerCase();
...
function openRecordSaleModal(initialItem = null) {
  ...
  const matchingItems = (query) => {
    ...
    if (!words.length) return availableItems.slice(0, 15);
